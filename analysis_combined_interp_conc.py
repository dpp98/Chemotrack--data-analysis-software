...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 27 14:02:51 2026
...
# -*- coding: utf-8 -*-
"""
Created on Fri May 15 11:53:54 2026
...
concdfb = np.zeros(nlines,dtype=float)
...
for i in range(nlines):
    ...
    rodfb[i] = np.log10(100 * (rocf - rocb) / rocc[i])

    concf = conc[i] + grad[i]*0.005
    concb = conc[i] - grad[i]*0.005

    concdfb[i] = np.log10(100 * (concf - concb)/conc[i])
...
#plt.xscale('log')
plt.xlabel('log10(percentage difference in ligand concentration)',fontsize=14)
plt.ylabel('background receptor occupancy',fontsize=14)
vals = np.array([0.003, 0.01, 0.05, 0.1, 0.25, 0.5, 1, 2.0, 4.0, 10])
#plt.xticks(np.log10(vals), vals)
...
plt.savefig('combined_plot_conc_contour.svg')
